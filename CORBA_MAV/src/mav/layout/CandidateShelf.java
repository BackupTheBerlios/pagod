...
public class CandidateShelf extends JPanel {
    ...
    private void drawAvatarName(Graphics2D g2) {
        ...
        Candidat candidat = (Candidat)this.candidatList.get(avatarIndex);
        TextLayout layout = new TextLayout(candidat.nom()+" "+candidat.prenom(), avatarFont, context);
    ...
    private void drawAvatarDescription(Graphics2D g2)
    {
    	 Composite composite = g2.getComposite();
         g2.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER,
                                                    textAlphaLevel));
         ...
         FontRenderContext context = g2.getFontRenderContext();
         Candidat candidat = (Candidat)this.candidatList.get(avatarIndex);
         TextLayout layout = new TextLayout(candidat.description(), avatarFont, context);
         Rectangle2D bounds = layout.getBounds();
         ...
         jl.setText("pouet pouet");
         jl.paint(g2);
        g2.drawImage(textImage, (int) x, (int) y+50, null);
         g2.setComposite(composite);
    }
    ...
    public void scrollAndAnimate(DrawableAvatar avatar) {
        if (loadingDone) {
    ...
    public void loadChunk(int startRow, int endRow) {
        picturesFinder = new Thread(new PicturesFinderThread(startRow, endRow, picturesFinder));
    ...
    private class PicturesFinderThread implements Runnable {
        ...
        private PicturesFinderThread(int s, int e, Thread parent) {
            startRow = s;
            endRow = e;
            this.parent = parent;
        }
        ...
        public void run() {
            ...
            for (int i = startRow; i <= endRow; i++) {
                try {
                    Thread.yield();
                    Candidat c = (Candidat)candidatList.get(i);
                    Image image = cache.getImageAndCache(c.nom());
                    avatars.add(image);
                } catch (Exception e) {
                	e.printStackTrace();
                }
                ...
                if (i == modelIndex + avatarAmount / 2) {
                    setAvatarIndex(i);
                    startPainting();
                }
