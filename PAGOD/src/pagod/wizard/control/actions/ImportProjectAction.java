 ...
 * Projet PAGOD
 * 
 * $Id: ImportProjectAction.java,v 1.2 2006/03/19 14:47:56 flotueur Exp $
 */
        ...
        if (workspaceChooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION)
        {
            ...
            // On teste si le répertoire est un projet valide
         	if (Project.isValidProjectDirectory(selectedPath)){
        		...
        		File targetdir = new File(currentWorkspace.getAbsolutePath()+File.separator+selectedPath.getName());
        		targetdir.mkdir();
        		// On recopie le répertoire
        		if (FilesManager.copyDirectory(selectedPath,targetdir,targetdir) 
        				&& !selectedPath.equals(targetdir)
        				&& !targetdir.exists()){
	        		// Le répertoire a bien été copié, on affiche un message à l'utilisateur
					JOptionPane.showMessageDialog(ApplicationManager.getInstance().getMfPagod(), LanguagesManager.getInstance().getString("infoImportProject"),
							LanguagesManager.getInstance().getString("titleInfoImportProject"),JOptionPane.INFORMATION_MESSAGE);
        		}
        		else{
        			// Il y a eu une erreur durant la copie et l'import ne s'est pas fait, on avertit l'utilisateur
					JOptionPane.showMessageDialog(ApplicationManager.getInstance().getMfPagod(), LanguagesManager.getInstance().getString("ImportProjectException"),
							LanguagesManager.getInstance().getString("ImportProjectErrorTitle"),JOptionPane.ERROR_MESSAGE);

        		}
             }
         	else
         	{
         		// le répertoire n'est pas un projet valide, on prévient l'utilisateur
				JOptionPane.showMessageDialog(ApplicationManager.getInstance().getMfPagod(), LanguagesManager.getInstance().getString("ImportProjectNull"),
						LanguagesManager.getInstance().getString("ImportProjectErrorTitle"),JOptionPane.ERROR_MESSAGE);
         	}
        }
