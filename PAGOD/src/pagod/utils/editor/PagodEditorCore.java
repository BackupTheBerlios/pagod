 ...
 * 
 */
public class PagodEditorCore extends JPanel implements ActionListener,
		KeyListener, FocusListener, DocumentListener
{
	...
	private HTMLUtilities					htmlUtilities			= new HTMLUtilities(
																			this);
	...
	private final String[]					extsHTML				= { "html",
			"htm", "shtml"											};
	private final String[]					extsCSS					= { "css" };
	private final String[]					extsIMG					= { "gif",
			"jpg", "jpeg", "png"									};
	...
	public PagodEditorCore (String sDocument, String sStyleSheet,
			String sRawDocument, StyledDocument sdocSource, URL urlStyleSheet,
			...
			boolean hasSpellChecker, boolean multiBar, String toolbarSeq)
	{
		...
		if (this.secManager != null)
		{
			try
			{
				...
				this.sysClipboard = Toolkit.getDefaultToolkit()
						.getSystemClipboard();
			}
		...
		Translatrix
				.setBundleName("resources.languages.editor.LanguageResources");
		...
		this.htmlDoc = (ExtendedHTMLDocument) (this.htmlKit
		...
		this.jtpMain.getDocument().addUndoableEditListener(
		...
		this.actionFontStrike = new FormatAction(this, Translatrix
				.getTranslationString("FontStrike"), HTML.Tag.STRIKE);
		this.actionFontSuperscript = new FormatAction(this, Translatrix
				.getTranslationString("FontSuperscript"), HTML.Tag.SUP);
		this.actionFontSubscript = new FormatAction(this, Translatrix
				.getTranslationString("FontSubscript"), HTML.Tag.SUB);
		this.actionListUnordered = new ListAutomationAction(this, Translatrix
				.getTranslationString("ListUnordered"), HTML.Tag.UL);
		this.actionListOrdered = new ListAutomationAction(this, Translatrix
				.getTranslationString("ListOrdered"), HTML.Tag.OL);
		this.actionSelectFont = new SetFontFamilyAction(this,
				"[MENUFONTSELECTOR]");
		this.actionAlignLeft = new StyledEditorKit.AlignmentAction(Translatrix
				.getTranslationString("AlignLeft"), StyleConstants.ALIGN_LEFT);
		this.actionAlignCenter = new StyledEditorKit.AlignmentAction(
				Translatrix.getTranslationString("AlignCenter"),
				StyleConstants.ALIGN_CENTER);
		this.actionAlignRight = new StyledEditorKit.AlignmentAction(Translatrix
				.getTranslationString("AlignRight"), StyleConstants.ALIGN_RIGHT);
		this.actionAlignJustified = new StyledEditorKit.AlignmentAction(
				Translatrix.getTranslationString("AlignJustified"),
				StyleConstants.ALIGN_JUSTIFIED);
		this.actionInsertAnchor = new CustomAction(this, Translatrix
				.getTranslationString("InsertAnchor")
				+ this.menuDialog, HTML.Tag.A);

		/* Build the menus */
		/* FILE Menu */
		this.jMenuFile = new JMenu(Translatrix.getTranslationString("File"));
		htMenus.put(KEY_MENU_FILE, this.jMenuFile);
		JMenuItem jmiNew = new JMenuItem(Translatrix
				.getTranslationString("NewDocument"));
		jmiNew.setActionCommand("newdoc");
		jmiNew.addActionListener(this);
		jmiNew.setAccelerator(KeyStroke.getKeyStroke('N', KeyEvent.CTRL_MASK,
				false));
		if (showMenuIcons)
		{
		...
		// jmiOpenCSS.addActionListener(this); jMenuFile.add(jmiOpenCSS);
		// JMenuItem jmiOpenB64 = new
		// JMenuItem(Translatrix.getTranslationString("OpenBase64Document") +
		// menuDialog); jmiOpenB64.setActionCommand("openb64");
		// jmiOpenB64.addActionListener(this); jMenuFile.add(jmiOpenB64);
		// JMenuItem jmiSave = new
		// JMenuItem(Translatrix.getTranslationString("Save"));
		// jmiSave.setActionCommand("save"); jmiSave.addActionListener(this);
		...
		// jmiSaveB64.addActionListener(this); jMenuFile.add(jmiSaveB64);
		/*
		 * jMenuFile.addSeparator(); JMenuItem jmiSerialOut = new
		 * JMenuItem(Translatrix.getTranslationString("Serialize") +
		 * menuDialog); jmiSerialOut.setActionCommand("serialize");
		 * jmiSerialOut.addActionListener(this); jMenuFile.add(jmiSerialOut);
		 * JMenuItem jmiSerialIn = new
		 * JMenuItem(Translatrix.getTranslationString("ReadFromSer") +
		 * menuDialog); jmiSerialIn.setActionCommand("readfromser");
		 * jmiSerialIn.addActionListener(this); jMenuFile.add(jmiSerialIn);
		 */
		this.jMenuFile.addSeparator();
		JMenuItem jmiExit = new JMenuItem(Translatrix
				.getTranslationString("Exit"));
		jmiExit.setActionCommand("exit");
		jmiExit.addActionListener(this);
		this.jMenuFile.add(jmiExit);
		...
		if (this.sysClipboard != null)
		{
			...
			JMenuItem jmiCut = new JMenuItem(Translatrix
					.getTranslationString("Cut"));
			...
			this.jMenuEdit.add(jmiCut);
			JMenuItem jmiCopy = new JMenuItem(Translatrix
					.getTranslationString("Copy"));
			...
			this.jMenuEdit.add(jmiCopy);
			JMenuItem jmiPaste = new JMenuItem(Translatrix
					.getTranslationString("Paste"));
		...
		JMenuItem jmiUndo = new JMenuItem(this.undoAction);
		jmiUndo.setAccelerator(KeyStroke.getKeyStroke('Z', KeyEvent.CTRL_MASK,
				false));
		if (showMenuIcons)
		{
			jmiUndo.setIcon(getEkitIcon("Undo"));
		}
		this.jMenuEdit.add(jmiUndo);
		JMenuItem jmiRedo = new JMenuItem(this.redoAction);
		jmiRedo.setAccelerator(KeyStroke.getKeyStroke('Y', KeyEvent.CTRL_MASK,
				false));
		if (showMenuIcons)
		{
			jmiRedo.setIcon(getEkitIcon("Redo"));
		}
		this.jMenuEdit.add(jmiRedo);
		this.jMenuEdit.addSeparator();
		...
		if (includeToolBar)
		{
			if (multiBar)
			{
				this.jMenuToolbars = new JMenu(Translatrix
						.getTranslationString("ViewToolbars"));

				this.jcbmiViewToolbarMain = new JCheckBoxMenuItem(Translatrix
				...
				this.jcbmiViewToolbarFormat = new JCheckBoxMenuItem(Translatrix
						.getTranslationString("ViewToolbarFormat"), false);
				this.jcbmiViewToolbarFormat
						.setActionCommand("toggletoolbarformat");
				this.jcbmiViewToolbarFormat.addActionListener(this);
				this.jMenuToolbars.add(this.jcbmiViewToolbarFormat);

				this.jcbmiViewToolbarStyles = new JCheckBoxMenuItem(Translatrix
						.getTranslationString("ViewToolbarStyles"), false);
				this.jcbmiViewToolbarStyles
						.setActionCommand("toggletoolbarstyles");
			...
			{
				this.jcbmiViewToolbar = new JCheckBoxMenuItem(Translatrix
		...
		this.jcbmiViewSource = new JCheckBoxMenuItem(Translatrix
		...
		JMenuItem jmiBold = new JMenuItem(this.actionFontBold);
		jmiBold.setText(Translatrix.getTranslationString("FontBold"));
		jmiBold.setAccelerator(KeyStroke.getKeyStroke('B', KeyEvent.CTRL_MASK,
				false));
		if (showMenuIcons)
		{
			jmiBold.setIcon(getEkitIcon("Bold"));
		}
		this.jMenuFont.add(jmiBold);
		JMenuItem jmiItalic = new JMenuItem(this.actionFontItalic);
		...
		this.jMenuFont.add(jmiItalic);
		JMenuItem jmiUnderline = new JMenuItem(this.actionFontUnderline);
		...
		this.jMenuFont.add(jmiUnderline);
		JMenuItem jmiStrike = new JMenuItem(this.actionFontStrike);
		jmiStrike.setText(Translatrix.getTranslationString("FontStrike"));
		if (showMenuIcons)
		{
			jmiStrike.setIcon(getEkitIcon("Strike"));
		}
		this.jMenuFont.add(jmiStrike);
		JMenuItem jmiSupscript = new JMenuItem(this.actionFontSuperscript);
		if (showMenuIcons)
		{
			jmiSupscript.setIcon(getEkitIcon("Super"));
		}
		this.jMenuFont.add(jmiSupscript);
		JMenuItem jmiSubscript = new JMenuItem(this.actionFontSubscript);
		if (showMenuIcons)
		{
			jmiSubscript.setIcon(getEkitIcon("Sub"));
		}
		this.jMenuFont.add(jmiSubscript);
		this.jMenuFont.addSeparator();
		this.jMenuFont.add(new JMenuItem(new FormatAction(this, Translatrix
				.getTranslationString("FormatBig"), HTML.Tag.BIG)));
		this.jMenuFont.add(new JMenuItem(new FormatAction(this, Translatrix
				.getTranslationString("FormatSmall"), HTML.Tag.SMALL)));
		JMenu jMenuFontSize = new JMenu(Translatrix
		...
		JMenuItem jmiSelectFont = new JMenuItem(this.actionSelectFont);
		...
		this.jMenuFont.addSeparator();
		JMenu jMenuFontColor = new JMenu(Translatrix
		...
		jMenuFontColor.add(new JMenuItem(new CustomAction(this, Translatrix
				...
				+ this.menuDialog, HTML.Tag.FONT, customAttr)));
		...
		JMenu jMenuFormatAlign = new JMenu(Translatrix
				.getTranslationString("Align"));
		JMenuItem jmiAlignLeft = new JMenuItem(this.actionAlignLeft);
		if (showMenuIcons)
		{
			jmiAlignLeft.setIcon(getEkitIcon("AlignLeft"));
		}
		jMenuFormatAlign.add(jmiAlignLeft);
		JMenuItem jmiAlignCenter = new JMenuItem(this.actionAlignCenter);
		if (showMenuIcons)
		{
			jmiAlignCenter.setIcon(getEkitIcon("AlignCenter"));
		}
		jMenuFormatAlign.add(jmiAlignCenter);
		JMenuItem jmiAlignRight = new JMenuItem(this.actionAlignRight);
		if (showMenuIcons)
		{
			jmiAlignRight.setIcon(getEkitIcon("AlignRight"));
		}
		jMenuFormatAlign.add(jmiAlignRight);
		JMenuItem jmiAlignJustified = new JMenuItem(this.actionAlignJustified);
		if (showMenuIcons)
		{
		...
		JMenu jMenuFormatHeading = new JMenu(Translatrix
		...
		this.jMenuFormat.addSeparator();
		// menu unordered list
		JMenuItem jmiUList = new JMenuItem(this.actionListUnordered);
		if (showMenuIcons)
		{
			jmiUList.setIcon(getEkitIcon("UList"));
		}
		this.jMenuFormat.add(jmiUList);
		// this.jMenuFormat.add(new JMenuItem(new FormatAction(this,
		...
		if (showMenuIcons)
		{
			...
			this.jMenuFormat.add(jmiOList);
			this.jMenuFormat.addSeparator();
			this.jMenuFormat.add(new JMenuItem(new FormatAction(this,
					Translatrix.getTranslationString("FormatBlockquote"),
					HTML.Tag.BLOCKQUOTE)));
			this.jMenuFormat.add(new JMenuItem(
					new FormatAction(this, Translatrix
							.getTranslationString("FormatPre"), HTML.Tag.PRE)));
			this.jMenuFormat.add(new JMenuItem(new FormatAction(this,
					Translatrix.getTranslationString("FormatStrong"),
					HTML.Tag.STRONG)));
			this.jMenuFormat.add(new JMenuItem(new FormatAction(this,
					Translatrix.getTranslationString("FormatEmphasis"),
					HTML.Tag.EM)));
			this.jMenuFormat
					.add(new JMenuItem(new FormatAction(this, Translatrix
							.getTranslationString("FormatTT"), HTML.Tag.TT)));
			this.jMenuFormat.add(new JMenuItem(new FormatAction(this,
					Translatrix.getTranslationString("FormatSpan"),
					HTML.Tag.SPAN)));

			/* INSERT Menu */
			this.jMenuInsert = new JMenu(Translatrix
					.getTranslationString("Insert"));
			htMenus.put(KEY_MENU_INSERT, this.jMenuInsert);
			JMenuItem jmiInsertAnchor = new JMenuItem(this.actionInsertAnchor);
			if (showMenuIcons)
			{
				jmiInsertAnchor.setIcon(getEkitIcon("Anchor"));
			}
			this.jMenuInsert.add(jmiInsertAnchor);
			JMenuItem jmiBreak = new JMenuItem(Translatrix
					.getTranslationString("InsertBreak"));
			...
			this.jMenuInsert.add(jmiBreak);
			JMenuItem jmiNBSP = new JMenuItem(Translatrix
					.getTranslationString("InsertNBSP"));
			...
			jmiNBSP.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_SPACE,
					KeyEvent.SHIFT_MASK, false));
			this.jMenuInsert.add(jmiNBSP);
			JMenu jMenuUnicode = new JMenu(Translatrix
			...
			this.jMenuInsert.add(jmiHRule);
			/*
			 * jMenuInsert.addSeparator(); JMenuItem jmiImageLocal = new
			 * JMenuItem(Translatrix.getTranslationString("InsertLocalImage") +
			 * menuDialog); jmiImageLocal.setActionCommand("insertlocalimage");
			 * jmiImageLocal.addActionListener(this);
			 * jMenuInsert.add(jmiImageLocal); JMenuItem jmiImageURL = new
			 * JMenuItem(Translatrix.getTranslationString("InsertURLImage") +
			 * menuDialog); jmiImageURL.setActionCommand("inserturlimage");
			 * jmiImageURL.addActionListener(this);
			 * jMenuInsert.add(jmiImageURL);
			 */

			/* TABLE Menu */
			this.jMenuTable = new JMenu(Translatrix
					.getTranslationString("Table"));
			htMenus.put(KEY_MENU_TABLE, this.jMenuTable);
			JMenuItem jmiTable = new JMenuItem(Translatrix
			...
			this.jMenuTable.add(jmiTable);
			this.jMenuTable.addSeparator();
			JMenuItem jmiTableRow = new JMenuItem(Translatrix
			...
			this.jMenuTable.add(jmiTableRow);
			JMenuItem jmiTableCol = new JMenuItem(Translatrix
			...
			this.jMenuTable.add(jmiTableCol);
			this.jMenuTable.addSeparator();
			JMenuItem jmiTableRowDel = new JMenuItem(Translatrix
			...
			this.jMenuTable.add(jmiTableRowDel);
			JMenuItem jmiTableColDel = new JMenuItem(Translatrix
			...
			this.jMenuTable.add(jmiTableColDel);

			/* FORMS Menu */
			/*
			 * jMenuForms = new
			 * JMenu(Translatrix.getTranslationString("Forms"));
			 * htMenus.put(KEY_MENU_FORMS, jMenuForms); JMenuItem
			 * jmiFormInsertForm = new
			 * JMenuItem(Translatrix.getTranslationString("FormInsertForm"));
			 * jmiFormInsertForm.setActionCommand("insertform");
			 * jmiFormInsertForm.addActionListener(this);
			 * jMenuForms.add(jmiFormInsertForm); jMenuForms.addSeparator();
			 * JMenuItem jmiFormTextfield = new
			 * JMenuItem(Translatrix.getTranslationString("FormTextfield"));
			 * jmiFormTextfield.setActionCommand("inserttextfield");
			 * jmiFormTextfield.addActionListener(this);
			 * jMenuForms.add(jmiFormTextfield); JMenuItem jmiFormTextarea = new
			 * JMenuItem(Translatrix.getTranslationString("FormTextarea"));
			 * jmiFormTextarea.setActionCommand("inserttextarea");
			 * jmiFormTextarea.addActionListener(this);
			 * jMenuForms.add(jmiFormTextarea); JMenuItem jmiFormCheckbox = new
			 * JMenuItem(Translatrix.getTranslationString("FormCheckbox"));
			 * jmiFormCheckbox.setActionCommand("insertcheckbox");
			 * jmiFormCheckbox.addActionListener(this);
			 * jMenuForms.add(jmiFormCheckbox); JMenuItem jmiFormRadio = new
			 * JMenuItem(Translatrix.getTranslationString("FormRadio"));
			 * jmiFormRadio.setActionCommand("insertradiobutton");
			 * jmiFormRadio.addActionListener(this);
			 * jMenuForms.add(jmiFormRadio); JMenuItem jmiFormPassword = new
			 * JMenuItem(Translatrix.getTranslationString("FormPassword"));
			 * jmiFormPassword.setActionCommand("insertpassword");
			 * jmiFormPassword.addActionListener(this);
			 * jMenuForms.add(jmiFormPassword); jMenuForms.addSeparator();
			 * JMenuItem jmiFormButton = new
			 * JMenuItem(Translatrix.getTranslationString("FormButton"));
			 * jmiFormButton.setActionCommand("insertbutton");
			 * jmiFormButton.addActionListener(this);
			 * jMenuForms.add(jmiFormButton); JMenuItem jmiFormButtonSubmit =
			 * new
			 * JMenuItem(Translatrix.getTranslationString("FormButtonSubmit"));
			 * jmiFormButtonSubmit.setActionCommand("insertbuttonsubmit");
			 * jmiFormButtonSubmit.addActionListener(this);
			 * jMenuForms.add(jmiFormButtonSubmit); JMenuItem jmiFormButtonReset =
			 * new
			 * JMenuItem(Translatrix.getTranslationString("FormButtonReset"));
			...
			jmiFind.setAccelerator(KeyStroke.getKeyStroke('F',
					KeyEvent.CTRL_MASK, false));
			this.jMenuSearch.add(jmiFind);
			JMenuItem jmiFindAgain = new JMenuItem(Translatrix
					.getTranslationString("SearchFindAgain"));
			if (showMenuIcons)
			{
			...
			jmiFindAgain.setAccelerator(KeyStroke.getKeyStroke('G',
					KeyEvent.CTRL_MASK, false));
			this.jMenuSearch.add(jmiFindAgain);
			JMenuItem jmiReplace = new JMenuItem(Translatrix
					.getTranslationString("SearchReplace"));
			if (showMenuIcons)
			{
			...
			jmiReplace.setAccelerator(KeyStroke.getKeyStroke('R',
					KeyEvent.CTRL_MASK, false));
			this.jMenuSearch.add(jmiReplace);

			/* HELP Menu */
			this.jMenuHelp = new JMenu(Translatrix.getTranslationString("Help"));
			htMenus.put(KEY_MENU_HELP, this.jMenuHelp);
			JMenuItem jmiAbout = new JMenuItem(Translatrix
					.getTranslationString("About"));
			jmiAbout.setActionCommand("helpabout");
			jmiAbout.addActionListener(this);
			this.jMenuHelp.add(jmiAbout);

			/* DEBUG Menu */
			this.jMenuDebug = new JMenu(Translatrix
					.getTranslationString("Debug"));
			htMenus.put(KEY_MENU_DEBUG, this.jMenuDebug);
			JMenuItem jmiDesc = new JMenuItem(Translatrix
					.getTranslationString("DescribeDoc"));
			jmiDesc.setActionCommand("describe");
			jmiDesc.addActionListener(this);
			this.jMenuDebug.add(jmiDesc);
			JMenuItem jmiDescCSS = new JMenuItem(Translatrix
					.getTranslationString("DescribeCSS"));
			jmiDescCSS.setActionCommand("describecss");
			jmiDescCSS.addActionListener(this);
			this.jMenuDebug.add(jmiDescCSS);
			JMenuItem jmiTag = new JMenuItem(Translatrix
					.getTranslationString("WhatTags"));
			jmiTag.setActionCommand("whattags");
			jmiTag.addActionListener(this);
			this.jMenuDebug.add(jmiTag);
			...
			this.jbtnNewHTML.setToolTipText(Translatrix
					.getTranslationString("NewDocument"));
			...
			this.jbtnUndo.setToolTipText(Translatrix
			...
			this.jbtnRedo.setToolTipText(Translatrix
			...
			this.jbtnBold.setToolTipText(Translatrix
					.getTranslationString("FontBold"));
			...
			this.jbtnItalic.setToolTipText(Translatrix
			...
			this.jbtnUnderline.setToolTipText(Translatrix
			...
			this.jbtnStrike.setToolTipText(Translatrix
					.getTranslationString("FontStrike"));
			htTools.put(KEY_TOOL_STRIKE, this.jbtnStrike);
			this.jbtnSuperscript = new JButtonNoFocus(
					this.actionFontSuperscript);
			this.jbtnSuperscript.setIcon(getEkitIcon("Super"));
			this.jbtnSuperscript.setText(null);
			this.jbtnSuperscript.setToolTipText(Translatrix
			...
			this.jbtnSubscript.setToolTipText(Translatrix
			...
			this.jbtnUList.setToolTipText(Translatrix
			...
			this.jbtnOList.setToolTipText(Translatrix
			...
			this.jbtnAlignLeft.setToolTipText(Translatrix
			...
			this.jbtnAlignCenter.setToolTipText(Translatrix
			...
			this.jbtnUnicode.setToolTipText(Translatrix
			...
			this.jbtnUnicodeMath.setToolTipText(Translatrix
			...
			this.jbtnFind.setToolTipText(Translatrix
					.getTranslationString("SearchFind"));
			...
			this.jbtnAnchor.setToolTipText(Translatrix
					.getTranslationString("ToolAnchor"));
			htTools.put(KEY_TOOL_ANCHOR, this.jbtnAnchor);
			this.jtbtnViewSource = new JToggleButtonNoFocus(
					getEkitIcon("Source"));
			this.jtbtnViewSource.setText(null);
			this.jtbtnViewSource.setToolTipText(Translatrix
			...
			this.jtbtnViewSource.setPreferredSize(this.jbtnAnchor
					.getPreferredSize());
			this.jtbtnViewSource.setMinimumSize(this.jbtnAnchor
					.getMinimumSize());
			this.jtbtnViewSource.setMaximumSize(this.jbtnAnchor
					.getMaximumSize());
			htTools.put(KEY_TOOL_SOURCE, this.jtbtnViewSource);
			this.jcmbStyleSelector = new JComboBoxNoFocus();
			this.jcmbStyleSelector.setToolTipText(Translatrix
					.getTranslationString("PickCSSStyle"));
			this.jcmbStyleSelector.setAction(new StylesAction(
					this.jcmbStyleSelector));
			...
			this.jcmbFontSelector.setAction(new SetFontFamilyAction(this,
					"[EKITFONTSELECTOR]"));
	 ...
	 *            [String] Code string specifying the toolbar buttons to show.
	 */
	public PagodEditorCore (String sRawDocument, URL urlStyleSheet,
			boolean includeToolBar, boolean showViewSource,
			boolean showMenuIcons, boolean editModeExclusive, String sLanguage,
			...
			boolean multiBar, String toolbarSeq)
	{
		this(null, null, sRawDocument, (StyledDocument) null, urlStyleSheet,
				includeToolBar, showViewSource, showMenuIcons,
				editModeExclusive, sLanguage, sCountry, base64, false,
				hasSpellChecker, multiBar, toolbarSeq);
	}
	...
	public void actionPerformed (ActionEvent ae)
	{
		try
		{
			...
			{
				writeOut((HTMLDocument) (this.jtpMain.getDocument()),
						this.currentFile);
				updateTitle();
			}
			...
			{
				// arno : sauvegarde
				writeOutFragment((HTMLDocument) (this.jtpMain.getDocument()),
						"body");
			}
			...
			else if (command.equals("describe"))
			{
				...
				System.out.println("Content Type : "
						+ this.jtpMain.getContentType());
				System.out.println("Editor Kit   : "
						+ this.jtpMain.getEditorKit());
			...
			{
				this.jToolBarMain.setVisible(this.jcbmiViewToolbarMain
						.isSelected());
			}
			else if (command.equals("toggletoolbarformat"))
			{
				this.jToolBarFormat.setVisible(this.jcbmiViewToolbarFormat
						.isSelected());
			}
			else if (command.equals("toggletoolbarstyles"))
			{
				this.jToolBarStyles.setVisible(this.jcbmiViewToolbarStyles
						.isSelected());
			}
			...
			{
				// arno : appel res bundle
				String[] fieldNames = {
						Translatrix.getTranslationString("TableRows"),
						Translatrix.getTranslationString("TableColumns"),
						Translatrix.getTranslationString("TableBorder"),
						Translatrix.getTranslationString("TableCellSpacing"),
						Translatrix.getTranslationString("TableCellPadding"),
						Translatrix.getTranslationString("Width") };
			...
			else if (command.equals("find"))
			{
				doSearch((String) null, (String) null, false,
						this.lastSearchCaseSetting, this.lastSearchTopSetting);
			}
			else if (command.equals("findagain"))
			{
				doSearch(this.lastSearchFindTerm, (String) null, false,
		...
		if (ke.getKeyChar() == KeyEvent.VK_BACK_SPACE)
		{
					...
					else
					{
						...
						if (sOffset == this.jtpMain.getSelectionStart())
						{
							...
							if (this.htmlUtilities.checkParentsTag(HTML.Tag.LI))
							{
								...
								if (so + 1 < eo)
								{
									char[] temp = this.jtpMain.getText(so,
											eo - so).toCharArray();
								...
								{
									this.jtpMain.setCaretPosition(this.jtpMain
											.getCaretPosition() - 1);
									this.jtpMain.moveCaretPosition(this.jtpMain
											.getCaretPosition() - 2);
									this.jtpMain.replaceSelection("");
									return;
								}
							}
							else if (this.htmlUtilities
			...
			catch (IOException ioe)
			{
				...
				SimpleInfoDialog sidAbout = new SimpleInfoDialog(this
						.getFrame(), Translatrix.getTranslationString("Error"),
						true, Translatrix
						...
						| this.htmlUtilities.checkParentsTag(HTML.Tag.OL) == true)
				{
					...
					else
					{
						...
						else
						{
							...
							Element newLi = this.htmlUtilities
									.getListItemParent();
	...
	public void registerDocumentStyles ()
	{
		...
		this.jcmbStyleSelector.addItem(Translatrix
	...
	public void insertListStyle (Element element) throws BadLocationException, IOException
	{
		...
		if (element.getParentElement().getName() == "ol")
		{
			this.actionListOrdered.actionPerformed(new ActionEvent(
					new Object(), 0, "newListPoint"));
		}
		else
		{
			this.actionListUnordered.actionPerformed(new ActionEvent(
					new Object(), 0, "newListPoint"));
		}
	}
			...
			RuntimeException, NumberFormatException
	{
		...
		if (fieldNames != null && fieldNames.length > 0)
		{
			PropertiesDialog propertiesDialog = new PropertiesDialog(this
					.getFrame(), fieldNames, fieldTypes, Translatrix
		...
		this.htmlKit.insertHTML(this.htmlDoc, caretPos, compositeElement
				.toString(), 0, 0, HTML.Tag.TABLE);
	...
	private void insertTableRow ()
	{
		...
		if (startPoint > -1 && columnCount > -1)
		{
			...
			sRow.append("</TR>");
			ActionEvent actionEvent = new ActionEvent(this.jtpMain, 0,
					"insertTableRow");
		...
		while (elementParent != null && !elementParent.getName().equals("body"))
		{
			...
			else if (elementParent.getName().equals("tr"))
			{
				...
				for (int i = 0; i < rowCells; i++)
				{
					...
					if (this.jtpMain.getCaretPosition() >= currentCell
							.getStartOffset()
							&& this.jtpMain.getCaretPosition() <= currentCell
									.getEndOffset())
					{
						cellOffset = i;
					}
		...
		if (elementCell != null && elementRow != null && elementTable != null)
		{
			...
			if (whichColumn > -1)
			{
				...
				for (int i = 0; i < elementTable.getElementCount(); i++)
				{
					...
					this.htmlDoc.remove(columnCellStart, columnCellEnd
							- columnCellStart);
				}
				this.jtpMain.setDocument(this.htmlDoc);
				registerDocument(this.htmlDoc);
	...
	{
		UnicodeDialog unicodeInput = new UnicodeDialog(this, Translatrix
			...
			BadLocationException, RuntimeException
	{
		...
		if (sChar != null)
		{
			this.htmlDoc.insertString(caretPos, sChar, this.jtpMain
					.getInputAttributes());
			this.jtpMain.setCaretPosition(caretPos + 1);
		}
	}
	...
	{
		int caretPos = this.jtpMain.getCaretPosition();
		this.htmlDoc.insertString(caretPos, "\240", this.jtpMain
				.getInputAttributes());
		this.jtpMain.setCaretPosition(caretPos + 1);
	}
			...
			BadLocationException, RuntimeException
	{
		...
		{
			PropertiesDialog propertiesDialog = new PropertiesDialog(this
					.getFrame(), fieldNames, fieldTypes, fieldValues,
			...
			BadLocationException, RuntimeException
	{
		if (whatImage == null)
		{
			whatImage = getImageFromChooser(this.imageChooserStartDir,
					this.extsIMG, Translatrix
		...
		if (whatImage != null)
		{
			...
			int caretPos = this.jtpMain.getCaretPosition();
			this.htmlKit.insertHTML(this.htmlDoc, caretPos, "<IMG SRC=\""
					+ whatImage + "\">", 0, 0, HTML.Tag.IMG);
			this.jtpMain.setCaretPosition(caretPos + 1);
			refreshOnUpdate();
		}
	}
	...
	public void insertURLImage () throws IOException, BadLocationException,
			RuntimeException
	{
		ImageURLDialog iurlDialog = new ImageURLDialog(this.getFrame(),
			...
			BadLocationException
	{
		File whatFile = getFileFromChooser(".", JFileChooser.SAVE_DIALOG,
				this.extsRTF, Translatrix.getTranslationString("FiletypeRTF"));
		if (whatFile != null)
		{
	...
	{
		File whatFile = getFileFromChooser(".", JFileChooser.SAVE_DIALOG,
				this.extsB64, Translatrix.getTranslationString("FiletypeB64"));
		if (whatFile != null)
		{
			...
			throws IOException, BadLocationException
	{
		...
		this.htmlDoc = (ExtendedHTMLDocument) (this.htmlKit
				.createDefaultDocument());
		this.htmlDoc.putProperty("pagod.utils.editor.docsource", whatFile
				.toString());
		try
		{
			...
			else
			{
				r = new InputStreamReader(new FileInputStream(whatFile),
						whatEncoding);
				this.htmlDoc.putProperty("IgnoreCharsetDirective", new Boolean(
						true));
			}
			...
			BadLocationException
	{
		...
		if (whatFile != null)
		{
			...
			try
			{
				...
				fr.close();
				this.jtpSource.setText(Base64Codec.decode(encodedText
						.toString()));
				this.jtpMain.setText(this.jtpSource.getText());
				registerDocument((ExtendedHTMLDocument) (this.jtpMain
						.getDocument()));
			}
			finally
			{
	...
	{
		File whatFile = getFileFromChooser(".", JFileChooser.SAVE_DIALOG,
				this.extsSer, Translatrix.getTranslationString("FiletypeSer"));
		if (whatFile != null)
		{
	 ...
	 * dialog
	 */
	private File getImageFromChooser (String startDir, String[] exts,
			String desc)
	{
		...
		int optionSelected = JFileChooser.CANCEL_OPTION;
		optionSelected = jImageDialog.showDialog(this, Translatrix
				.getTranslationString("Insert"));
		if (optionSelected == JFileChooser.APPROVE_OPTION)
		{
	...
	public void setDocumentText (String sText)
	{
		...
		((HTMLEditorKit) (this.jtpMain.getEditorKit()))
				.setDefaultCursor(new Cursor(Cursor.TEXT_CURSOR));
		this.jtpSource.setText(this.jtpMain.getText());
	}
	...
	{
		if (this.jcmbFontSelector == null
				|| this.jcmbFontSelector
						.getSelectedItem()
						.equals(
								Translatrix
										.getTranslationString("SelectorToolFontsDefaultFont")))
	...
	{
		this.frameHandler.setTitle(this.appName
				+ (this.currentFile == null ? "" : " - "
	...
	private ImageIcon getEkitIcon (String iconName)
	{
		URL imageURL = getClass().getResource(
				"/resources/images/pagodeditoricons/" + iconName + "HK.png");
		if (imageURL != null)
		{
			return new ImageIcon(Toolkit.getDefaultToolkit().getImage(imageURL));
		}
		imageURL = getClass().getResource(
				"/resources/images/pagodeditoricons/" + iconName + "HK.gif");
		if (imageURL != null)
		{
			return new ImageIcon(Toolkit.getDefaultToolkit().getImage(imageURL));
		}
	...
	{
		return (this.jspSource != null && this.jspSource == this.jspltDisplay
				.getRightComponent());
	}
	...
	private void handleCaretPositionChange (CaretEvent ce)
	{
		...
		if (this.jtpMain.hasFocus())
		{
			...
			if (this.jcmbFontSelector != null
					&& this.jcmbFontSelector.isVisible())
			{
				...
				this.jcmbFontSelector.getModel()
						.setSelectedItem(activeFontName);
	...
	protected void write ()
	{
		int choice = new JOptionPane().showConfirmDialog(this, Translatrix
				.getTranslationString("SaveBeforeQuit"));
		if (choice == 0) try
		{
			writeOutFragment((HTMLDocument) (this.jtpMain.getDocument()),
					"body");
