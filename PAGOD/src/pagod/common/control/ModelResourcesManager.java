/*
 * $Id: ModelResourcesManager.java,v 1.1 2005/10/30 10:44:59 yak Exp $
 *
...
public class ModelResourcesManager
{
    ...
    public Image getDefaultImage(ProcessElement processElement)
    {
        ...
        if (processElement.getClass() == Role.class)
            ImageToReturned = imageManager.getImageResource("RoleIcon.gif");
        else if (processElement.getClass() == Activity.class)
            ImageToReturned = imageManager.getImageResource("ActivityIcon.gif");
        else if (processElement.getClass() == WorkDefinition.class)
            ImageToReturned = imageManager
                    .getImageResource("WorkDefinitionIcon.gif");
        else if (processElement.getClass() == ProcessComponent.class)
            ImageToReturned = imageManager
                    .getImageResource("ProcessComponentIcon.gif");
        else if (processElement.getClass() == Guidance.class)
            ImageToReturned = imageManager.getImageResource("GuidanceIcon.gif");
        else if (processElement.getClass() == Product.class)
            ImageToReturned = imageManager.getImageResource("ProductIcon.gif");
        else if (processElement.getClass() == Process.class)
            ImageToReturned = imageManager.getImageResource("ProcessIcon.gif");
        else
        ...
        if (file != null)
        {
            ...
            if (fileStream!=null)
            {
                ...
                if (PreferencesManager.getInstance().containPreference(sExt))
                {
                    try
                    {
                        sCommand = PreferencesManager.getInstance().getPreference(
                                sExt);
                    }
...
//                      TODO gerer cette erreur
    ...
    public void launchProductApplication(Product product)
    {
        ...
        {
                JOptionPane.showMessageDialog(null, LanguagesManager
                        .getInstance().getString("nonDefineTool"),
                        LanguagesManager.getInstance().getString(
                                "nonDefineToolTitle"),
                        JOptionPane.WARNING_MESSAGE);
        }
